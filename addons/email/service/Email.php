...
class Email extends Service
{
    ...
    public function send($to, $event, $template_date, $from=''){
        ...
        $email_lib = \addons\email\library\Email::instance();
        if($from){
            $email_lib->from($from);
        }
        ...
        if($result){
            ...
            \addons\email\admin\model\Email::create($data);
            return true;
        }else{
    ...
    public function checkCode($to, $event, $code){
        $email = \addons\email\admin\model\Email::where([['to','=',$to],['event','=',$event],['params','=',json_encode(['code'=>$code],JSON_UNESCAPED_UNICODE)]])->find();
        if(!$email){
            $this->setError('验证码错误');
            return false;
        }
        ...
        if($email->expire_time && $email->expire_time < time()){
            \addons\email\admin\model\Email::where('id','=',$email->id)->update(['status'=>3]);
            $this->setError('验证码已过期');
            return false;
        }
        \addons\email\admin\model\Email::where('id','=',$email->id)->update(['status'=>2]);
        return true;
    }
}