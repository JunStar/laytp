    ...
    protected function get_name_by_table($table){
        ...
        if(count($arr_table)){
            ...
            if($str_table_temp == 'admin' && $basename == 'User'){
                ...
                $str_table = implode('\\', $arr_table) . '\\' . $basename;
            }
        }else{
            $str_table = $basename;
        }
    ...
    protected function set_relation_model(){
        ...
        if(is_array($relation_model) && count($relation_model)){
            foreach($relation_model as $k=>$item){
                ...
                $show_field = explode(',',$item['show_field']);
                if(!in_array('id',$show_field)){
                    $item['show_field'] = implode(',',array_merge(['id'],$show_field));
                }
    ...
    protected function set_html_param(){
        ...
        foreach($this->curd_config['field_list'] as $k=>$v){
            if( in_array( $v['form_type'], ['select_relation'] ) ){
                ...
                $edit_data[] = $this->get_form_item($v,'edit');
            }else{
                if($v['field_show_add']){
