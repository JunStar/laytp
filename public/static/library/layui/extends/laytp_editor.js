layui.define(['jquery'], function(exports){
        ...
        content: function(content){
            ...
            //转义图片
                .replace(/img\[([^\s]+?)\]/g, function(img){
                    return '<img src="' + img.replace(/(^img\[)|(\]$)/g, '') + '" width="100%" height="100%">';
                })
    ...
    //创建编辑器
    laytp_editor.createEditor= function(options){
        ...
        var log = {}, mod = {
            face: function(editor, self){ //插入表情
                var str = '', ul, face = laytp_editor.facade.faces;
                for(var key in face){
                ...
                $('#LAY-editface li').on('click', function(){
                    ...
                    laytp_editor.facade.focusInsert(editor[0], 'face' + title);
            ...
            ,picture: function(editor){ //插入图片
                layer.open({
                    ...
                    ,success: function(layero, index){
                        ...
                        layui.upload.render({
                            elem: '#uploadImg'
                            ,url: laytp_editor.facade.url('/admin/ajax/upload',{'accept':'images'})
                            ,size: 300000000
                            ,done: function(res){
                                if(res.code == 1){
                        ...
                        layui.form.on('submit(uploadImages)', function(data){
                            ...
                            laytp_editor.facade.focusInsert(editor[0], 'img['+ field.image + '] ');
                            layer.close(index);
                            editor.trigger('keyup');
                        });
                    }
            ...
            ,href: function(editor){ //超链接
                layer.prompt({
                    ...
                    laytp_editor.facade.focusInsert(editor[0], ' a('+ val +')['+ val + '] ');
                    layer.close(index);
                    editor.trigger('keyup');
                });
            }
            ,quote: function(editor){ //引用
                layer.prompt({
                    ...
                    laytp_editor.facade.focusInsert(editor[0], '[quote]\n  '+ val + '\n[/quote]\n');
                    layer.close(index);
                    editor.trigger('keyup');
                });
            }
            ,table: function(editor){
                laytp_editor.facade.focusInsert(editor[0],
            '[table]\n  '+
                    '[thead]\n' +
                    '   [tr]\n' +
                    '       [th]表头1[/th]\n' +
            ...
            ,code: function(editor){ //插入代码
                layer.prompt({
                    ...
                    layui.laytp_editor.facade.focusInsert(editor[0], '[pre]\n'+ val + '\n[/pre]\n');
                    layer.close(index);
                    editor.trigger('keyup');
                });
            }
            ,hr: function(editor){ //插入水平分割线
                laytp_editor.facade.focusInsert(editor[0], '[hr]\n');
                editor.trigger('keyup');
            }
            ,preview: function(editor, span){ //预览
                var othis = $(span), getContent = function(){
                    ...
                    return /^\{html\}/.test(content)
                        ...
                        : laytp_editor.facade.content(content)
