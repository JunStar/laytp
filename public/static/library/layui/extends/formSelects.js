...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
	    ...
	    SEARCH_FIELD = 'xm-select-search-field',
	    SEARCH_CONDITION = 'xm-select-search-condition',
		...
		this.config = {
			...
			maxTips: function maxTips(id, vals, val, max) {
				...
				//弹窗要特殊处理
				layui.layer.tips('最多只能选择 '+max+' 个', ipt, {
					tips: 3,
					time: 1000,
					end:function(){}
				});
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			...
			var searchField = othis.attr(SEARCH_FIELD);
			var searchCondition = othis.attr(SEARCH_CONDITION);
	...
	Common.prototype.search = function (id, searchField, searchCondition, e, searchUrl, call) {
		var _this3 = this;
		...
		if (searchUrl) {
			...
			if (!ajaxConfig.beforeSearch || ajaxConfig.beforeSearch && ajaxConfig.beforeSearch instanceof Function && ajaxConfig.beforeSearch(id, searchUrl, inputValue)) {
				...
				fs.clearid = setTimeout(function () {
					...
					_this3.ajax(id, searchUrl, inputValue, searchField, searchCondition, false, null, true);
				}, delay);
			}
		} else {
	...
	Common.prototype.ajax = function (id, searchUrl, inputValue, searchField, searchCondition, isLinkage, linkageWidth, isSearch, successCallback, isReplace) {
		var _this5 = this;
		...
		if(searchField){
			...
			postData['search_param'][searchField] = {};
			postData['search_param'][searchField]['value'] = inputValue;
			postData['search_param'][searchField]['condition'] = searchCondition;
		}
		var ajaxData = $.extend(true, postData, ajaxConfig.data);
		//是否需要对ajax添加随机时间
		//ajaxData['_'] = Date.now();
		$.ajax({
