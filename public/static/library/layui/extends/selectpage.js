...
layui.define('jquery', function (exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
        ...
        var rePosition = function () {
            if (el.container.css('position') === 'static') {
                ...
                el.result_area.css({
                    ...
                    left: offset.left + 'px', 'z-index': '20000101'
                });
            } else {
                if (!p.pagination) {
                ...
                return {
                    top: top + 'px', left: left + 'px', 'z-index': '20000101'
                };
            }
        };
     ...
     * @constructor
     */
    function Plugin(obj, option) {
        if (!obj) return false;
        var $this = $(obj),
            data = $this.data(SelectPage.dataKey),
            params = $.extend({}, defaults, option.data, data && data.option, typeof option === 'object' && option);
        if (!data) {
            $this.data(SelectPage.dataKey, (data = new SelectPage($(obj), params)));
        }
        return data;
    }
     ...
     */
    function ModifyDataSource(obj, data) {
     ...
     */
    function PluginDisabled(obj, disabled) {
        var status = false;
        var $this = getPlugin(obj),
            plugin = $this.data(SelectPage.dataKey);
        if (plugin) {
    ...
    function GetInputText(obj) {
        ...
        var $this = getPlugin(obj), data = $this.data(SelectPage.dataKey);
        if (data) {
    ...
    var obj = {
        selectPage: Plugin
        , selectPageClear: ClearSelected
        , selectPageRefresh: SelectedRefresh
        , selectPageData: ModifyDataSource
        , selectPageDisabled: PluginDisabled
        , selectPageText: GetInputText
    };

    //输出接口
    exports('selectPage', obj);
});