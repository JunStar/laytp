layui.use(["laytp"], function () {
    const funController = {};
    //静态页面地址前缀
    window.htmlPrefix = facade.compatibleHtmlPath("/admin/test/category/");
    //后端接口地址前缀
    window.apiPrefix  = facade.compatibleApiRoute("/admin.test.category/");
    ...
    funController.tableRender = function (where) {
        ...
        laytpTreeTable = layui.treeTable.render({
            ...
            , url: facade.url("/admin.test.category/index", {is_tree:1})
            , where: where
            , method: "GET"
            ...
            , cols: [[
				...
				,{field:'pid',title:'上级分类',align:'center',templet:'<div>{{# if(d.parent){ }}{{d.parent.name}}{{# }else{ }}-{{# } }}</div>'}
				,{field:'sort',title:'排序',align:'center',templet:function(d){
                        return laytpForm.tableForm.editInput('sort',d,'/admin.test.category/setSort');
                    }}
