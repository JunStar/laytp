layui.define(['table', 'jquery', 'element','facade'], function(exports) {
	...
	function getData(route) {
		...
		// $.get(localStorage.getItem("adminApiDomain") + url + contact + "fresh=" + Math.random(), function(result) {
		// 	defer.resolve(result)
		// });
		...
		$.each(option.data, function(i, item) {
			...
			if(index < config.menu['maxTopMenuNum']){
                if (i === option.defaultMenu) {
                    controlItem = '<li menu-href="' + item.href + '" menu-title="' + item.title + '" menu-id="' + item.id +
	...
	function isHoverMenu(b, option) {
		if (b) {
			$("#" + option.elem + ".laytp-nav-mini .layui-nav-item,#" + option.elem + ".laytp-nav-mini dd").hover(function(e) {
				e.stopPropagation();
				var _this = $(this);
