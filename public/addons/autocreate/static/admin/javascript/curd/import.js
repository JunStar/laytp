layui.use(['layTp'],function(){
    ...
    func_controller.init = function(){
        ...
        layui.form.on('select(select_table)',function(data){
            //全局模型，隐藏主键列等渲染成非选中状态
            $('#common_model').prop('checked',false);
            $('#hide_pk').prop('checked',false);
            $('#create_number').prop('checked',false);
            $('#close_page').prop('checked',false);
            $('#hide_del').prop('checked',false);

            //关联模型设为非选中状态
            $('#relation_model').prop('checked',false);
            ...
            $.ajax({
                type: 'GET',
                url: layTp.facade.addon_url('autocreate','/' + module + '/' + controller + '/get_curd_info'),
                data: ajax_data,
                dataType: 'json',
                success: function (res) {
                    ...
                    if( res.code == 1 ){
                        ...
                        if(relation_model && relation_model.length>0){
                            $('#relation_model').prop('checked',true);
                        ...
                        if(typeof global != "undefined"){
                            if(global.common_model == 1){
                                $('#common_model').prop('checked',true);
                            }
                            if(global.hide_pk == 1){
                                $('#hide_pk').prop('checked',true);
                            }
                            if(global.create_number == 1){
                                $('#create_number').prop('checked',true);
                            }
                            if(global.close_page == 1){
                                $('#close_page').prop('checked',true);
                            }
                            if(global.hide_del == 1){
                                $('#hide_del').prop('checked',true);
                            }
        ...
        layui.form.on('select(select_relation_table)',function(data){
            ...
            $.ajax({
                type: 'GET',
                url: layTp.facade.addon_url('autocreate','admin/curd/get_fields_with_pk_by_table_name'),
            ...
            switch (value) {
                case 'select_page':
                    ...
                    layui.form.on('select(form_additional_select_page_table_'+field_name+')',function(data){
                        ...
                        $.ajax({
                            type: 'GET',
                            url: layTp.facade.addon_url('autocreate','admin/curd/get_fields_by_table_name'),
